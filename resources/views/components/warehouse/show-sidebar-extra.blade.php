...
@can('warehouses.manage')
    <section class="mi-guide-section">
        ...
        @else
            <form action="{{ route('warehouses.destroy', $warehouse) }}" method="POST" class="mt-3"
                  onsubmit="return confirm('Delete {{ addslashes($warehouse->name) }}? This cannot be undone easily.');">
                @csrf
                @method('DELETE')
