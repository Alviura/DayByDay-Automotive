    ...
    <div class="mi-page space-y-5">
        <div class="flex flex-wrap items-start justify-between gap-4">
            <div class="flex items-start gap-3">
                ...
                <div>
                    <h1 class="text-[1.35rem] font-bold">Supplier Returns</h1>
                    <p class="text-sm text-gray-500">Return defective stock to suppliers — removes from warehouse on approval.</p>
                </div>
            </div>
            @can('returns.create')
                <a href="{{ route('supplier-returns.create') }}" class="mi-btn-orange"><i class="fas fa-plus text-xs"></i> New Return</a>
            @endcan
        </div>
        ...
        <div class="mi-card">
            <div class="mi-table-wrap">
                <table class="mi-table">
                    <thead><tr><th>Return #</th><th>Supplier</th><th>Warehouse</th><th>Lines</th><th>Status</th><th></th></tr></thead>
                    <tbody>
                        @forelse ($returns as $return)
                            <tr>
                                <td><a href="{{ route('supplier-returns.show', $return) }}" class="mi-cat-badge">{{ $return->return_number }}</a></td>
                                <td>{{ $return->supplier?->name }}</td>
                                <td>{{ $return->warehouse?->name }}</td>
                                <td>{{ $return->items->count() }}</td>
                                <td>{{ $return->statusLabel() }}</td>
                                <td><a href="{{ route('supplier-returns.show', $return) }}" class="mi-action view"><i class="fas fa-eye"></i></a></td>
                            </tr>
                        @empty
                            <tr><td colspan="6" class="text-center py-12 text-gray-400">No supplier returns yet.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
