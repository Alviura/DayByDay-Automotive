<x-app-layout :title="'Receive '.$transfer->request_number">
    ...
    @php $st = $transfer->stockTransfer; @endphp
    ...
    <div class="mi-page space-y-5">
            ...
            <div class="flex items-start gap-3">
                ...
                <div>
                    ...
                    <p class="mt-0.5 text-sm text-gray-500">
                        ...
                        @if ($st) · <span class="font-mono">{{ $st->transfer_number }}</span> @endif
                        · {{ $transfer->routeLabel() }}
                    </p>
                </div>
            </div>
            <a href="{{ route('transfers.show', $transfer) }}" class="mi-btn-ghost">
        ...
        <div class="tr-show-banner">
            ...
            <p class="text-sm text-gray-600">Confirm quantities received at <strong>{{ $transfer->destinationLabel() }}</strong>. Good qty posts to shop inventory; damaged qty is recorded but not stocked.</p>
        </div>

        <form method="POST" action="{{ route('transfers.receive.store', $transfer) }}" class="mi-card">
            @csrf
            ...
            <div class="p-4 border-b border-gray-100">
                ...
                <textarea name="notes" rows="2" class="mi-input block w-full" placeholder="Optional — condition, partial delivery…">{{ old('notes') }}</textarea>
            </div>
            <div class="mi-table-wrap">
                <table class="mi-table">
                    ...
                    <tbody>
                        @foreach ($st->items as $index => $item)
